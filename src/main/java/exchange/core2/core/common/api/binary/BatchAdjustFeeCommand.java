...
@EqualsAndHashCode
@Getter
public final class BatchAdjustFeeCommand implements BinaryDataCommand {

    private final IntObjectHashMap<CoreSymbolSpecification> symbols;

    public BatchAdjustFeeCommand(final CoreSymbolSpecification symbol) {
        symbols = IntObjectHashMap.newWithKeysValues(symbol.symbolId, symbol);
    }

    public BatchAdjustFeeCommand(final Collection<CoreSymbolSpecification> collection) {
        symbols = new IntObjectHashMap<>(collection.size());
        collection.forEach(s -> symbols.put(s.symbolId, s));
    }


    public BatchAdjustFeeCommand(final BytesIn bytes) {
    ...
    public int getBinaryCommandTypeCode() {
        return BinaryCommandType.ADJUST_FEE.getCode();
    }
}
